// Pane tree data model + mutation logic (add/close/move tab, split, drop),
// ported from `origami`'s own `qml/pane/core/PaneTreeOps.qml` +
// `PaneView.qml`'s tree-mutation methods -- see each method's own doc
// comment for its specific QML counterpart.

...
// Generic, reusable bar shell: three content slots -- `start`
...
// Theme.header Rectangle), so this stays reusable for bars/headers that want no
...
// For a bar that needs more than one line, see MultiRowBar.qml
// instead -- it reuses this same start/center/end layout (BarRow.qml)
...
Item {
    ...
    // Forwards BarRow's own contentHeight (see its class comment) --
    ...
    // Forwards BarRow's own endWidth (see its class comment) so a
    // caller can work out how much room `start` actually has before
    // running into `end`.
    readonly property real endWidth: row.endWidth

    // Forwards BarRow's own centerWidth (see its class comment) so a
